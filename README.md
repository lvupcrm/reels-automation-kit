...
| | |
|---|---|
| 컴퓨터 | **맥 M1 이상** · **윈도우 10 이상 64비트** (둘 다 검증됨) |
| 클로드 | **Pro(월 $20) 이상** 구독 + [Claude Code 앱](https://claude.com/claude-code) |
| 메모리 | **16GB 권장** — 목소리 모델 로드에만 약 8GB를 쓴다 |
| 디스크 | 여유 20GB (AI 모델 6~8GB) |
| 그래픽카드 | 불필요 — 모든 처리가 CPU로 돈다 |
...
> 윈도우에서 `SoX could not be found!` 경고가 떠도 무시하면 된다 — 이 키트가 쓰는 12Hz 모델은 SoX를 사용하지 않는다.
